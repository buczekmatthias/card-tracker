const masteries = {
  ...
  "Super tower": { cost: 1000, description: "Causes 50% cards multiplier effect to increase all Ultimate Weapon damage and decreases Super Tower cooldown and unlocks Ultimate Tower mastery research", researchName: "Ultimate Tower", research: "Reduces Super Tower cooldown time" },
...
const getTotalCost = () => {
  ...
  Object.entries(masteries).forEach((content) => {
    total += content[1].cost;
  });

  return total;
};

const getSpentStones = (groups) => {
  const cards = [...groups[0].cards, ...groups[1].cards, ...groups[2].cards];
  let total = 0;

  cards.forEach((card) => {
    if (card.masteryUnlocked) {
      total += masteries[card.name].cost;
    }
  });
...
const getRequiredStones = (groups) => getTotalCost() - getSpentStones(groups);

export { masteries, getSpentStones, getRequiredStones };
